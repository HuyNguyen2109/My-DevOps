...
ssh -i "$KEY" "$TGT" "sudo cp -r $UP/* $REM/ && sudo chown 100:1000 $REM/bao-config.hcl && sudo chmod 600 $REM/bao-config.hcl && cd $REM && sudo docker compose up -d --force-recreate"